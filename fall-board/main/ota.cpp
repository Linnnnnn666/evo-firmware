...
static bool fw_newer(const char *remote) {
    ...
    int nl = sscanf(FW_VERSION, "v%d.%d.%d", &l[0], &l[1], &l[2]);
    if (nl < 3) nl = sscanf(FW_VERSION, "%d.%d.%d", &l[0], &l[1], &l[2]);
...
esp_err_t ota_check_now(void) {
    ...
    if (cJSON_IsString(v) && v->valuestring && fw_newer(v->valuestring)) {
        ESP_LOGW(TAG, "发现新版本 %s（当前 %s），开始 OTA", v->valuestring, FW_VERSION);
        cJSON *bin = cJSON_GetObjectItem(root, "bin");
        char url[256];
        snprintf(url, sizeof(url), "%s/firmware/fall-board/%s/%s",
                 DEVICE_API_BASE_URL, v->valuestring,
        ...
        ESP_LOGI(TAG, "固件已是最新（%s）", FW_VERSION);
    }
    cJSON_Delete(root);
    return result;
}
