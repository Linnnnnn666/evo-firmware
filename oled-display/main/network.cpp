// 轻量网络层：nvs + esp_netif + WiFi STA 连接（烧录器固件用）
#include "network.h"
...
bool network_init(void) {
    ...
    wifi_config_t sta = {};
    strlcpy(reinterpret_cast<char *>(sta.sta.ssid), WIFI_SSID, sizeof(sta.sta.ssid));
    strlcpy(reinterpret_cast<char *>(sta.sta.password), WIFI_PASSWORD, sizeof(sta.sta.password));
    ...
    g_started = true;
    ESP_LOGI(TAG, "Wi-Fi station started, SSID=%s", WIFI_SSID);
    return true;
}
